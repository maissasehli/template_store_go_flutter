...
void onInit() {
  ...
  print("ProductDetailControllerImp initialized");
  ...
  if (Get.arguments != null && Get.arguments is Map) {
    ...
    print("No arguments received or invalid arguments format");
  }
}
@override
Future<void> loadProductDetails(String productId) async {
  try {
    ...
    if (response.statusCode == 200 && response.data['status'] == 'success') {
      ...
      if (productModel.attributes != null) {
        if (productModel.attributes!.size != null && productModel.attributes!.size!.isNotEmpty) {
          selectedSize.value = productModel.attributes!.size![0];
        }
        
        if (productModel.attributes!.color != null && productModel.attributes!.color!.isNotEmpty) {
          selectedColor.value = productModel.attributes!.color![0];
        }
      }
      
      print("Product details loaded successfully: ${product.value!.name}");
    } else {
  ...
  void toggleFavorite() {
    if (product.value != null && product.value!.id != null) {
      ...
      _productApiService.toggleFavorite(product.value!.id!, newStatus).then((_) {
